pub mod solution {
    ...
    fn overlap(pair1: String, pair2: String) -> bool {
        let _pair1: Vec<u8> = pair1
            .split("-")
            .map(|range| -> u8 { range.parse::<u8>().unwrap_or(0) })
            .collect();
        ...
        if _pair1[1] >= _pair2[0] && _pair2[1] >= _pair1[0] {
            // println!("{:?}, {:?}, true", _pair1, _pair2);
            return true;
        }
        if _pair2[1] >= _pair1[0] && _pair1[1] >= _pair2[0] {
            // println!("{:?}, {:?}, true", _pair1, _pair2);
            return true;
        }
        return false;
    ...
    pub fn solve2() -> usize {
        ...
        for pair in range_pairs {
            if pair != "" {
                ...
                let left_pair = pair[0];
                let right_pair = pair[1];
                if overlap(String::from(left_pair), String::from(right_pair)) {
                    ctr = ctr + 1;
                }
            }
        }
